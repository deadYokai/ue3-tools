...
"#
    ...
    println!(
        "   ue3-tools mod extract <Pkg.upk> <ObjName> {} --dir <subdir>",
        mod_dir.display()
    );
...
pub fn cmd_extract(
    ...
    let found = pak.export_table.iter().enumerate().find(|(i, _)| {
        ...
        path.to_lowercase().contains(&needle) || full.to_lowercase().contains(&needle)
    });
    ...
    if e > raw.len() {
        return Err(io::Error::new(
            ...
            "serial data out of bounds",
        ));
    }
    fs::write(&blob_path, &raw[s..e])?;

    println!("✓  {} bytes → {}", exp.serial_size, blob_path.display());
...
pub fn cmd_pack(mod_dir: &Path, dist_dir: &Path) -> io::Result<()> {
    ...
    for block in &manifest.patch {
        let (bin, unc) = build_patch_bin(mod_dir, block)?;
        ...
        println!("✓  {} replace(s) → {}", block.replace.len(), out.display());
    }
...
pub fn load_manifest(mod_dir: &Path) -> io::Result<ModManifest> {
    ...
    let text = fs::read_to_string(&toml_path)
        .map_err(|e| io::Error::new(e.kind(), format!("{}: {e}", toml_path.display())))?;
    toml::from_str(&text).map_err(|e| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            format!("mod.toml parse error: {e}"),
        )
    })
}

fn build_patch_bin(mod_dir: &Path, block: &PatchBlock) -> io::Result<(Vec<u8>, usize)> {
    ...
    compress_patch(&lpd)
        .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("compress_patch: {e}")))
}
